...
class PPO:
    ...
    def __init__(self, env):
        ...
        self.cov_mat = 0.5 * torch.eye(self.act_dim) # covariance matrix for action distribution
        self.gamma = 0.95 # discount factor
        self.n_epochs_per_iteration = 5 # number of epochs of optimization of the actor per iteration TODO: can this be varied according to some statistics about the gradients?
    ...
    def rollout(self):
        ...
        TODO: Can it collect more than `timesteps_per_batch` timesteps?
        ...
        while t_batch < self.timesteps_per_batch: # TODO: can t_batch go above timesteps_per_batch?
    ...
    def evaluate(self, batch_obs, batch_acts):
        """
        Evaluates the value function of a batch of observations according to the critic.
        Also evaluates the log probability of the actions taken in the batch according to the actor and the covariance matrix.
        TODO: Why not split this into two functions?
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps:
            ...
            for epoch in range(self.n_epochs_per_iteration):
                ...
                # calculate pi_theta(a_t | s_t) for all timesteps in the batch
                V, current_log_probs = self.evaluate(batch_obs, batch_acts) # the V here overwrites the V from the previous epoch
                ratios = torch.exp(current_log_probs - batch_log_probs)
                
                # calculate loss
                surr_1 = ratios * A_k
                surr_2 = torch.clamp(ratios, 1 - self.clip, 1 + self.clip) * A_k
                actor_loss = -torch.min(surr_1, surr_2).mean() # i.e. we want to mazimise the minimum of the two surrogate losses
                
                # optimize actor
                # NOTE: the thing that changes in each epoch is the current_log_probs of the actions taken in the batch, which are used to calculate the ratios
